...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    rows.forEach((row: any) => {
      // Tạo một khóa duy nhất cho SESSION_ID và SESSION_START_TIME
      const sessionKey = `${row.SESSION_ID}_${row.SESSION_START_TIME}`;

      // Nếu SESSION_ID và SESSION_START_TIME chưa có trong sessionMap, khởi tạo nó
